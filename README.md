# Horizon_G4_LED_Control

1. 项目简介
Horizon_G4_UART_Control 是 Horizon-G4 计划的第二个里程碑。 本项目在 STM32G431 高性能开发板上，实现了基于 USART1 的双向串行通信系统。通过上位机（VOFA+）发送字符指令，实时控制板载 LED 的亮灭，并利用重定向的 printf 函数回传设备状态，构成了完整的**“指令-执行-反馈”**闭环链路。
...
重定向 printf：实现了 fputc 函数，支持通过 printf 直接向串口打印调试信息。

阻塞式接收：使用 HAL_UART_Receive 配合 10ms 超时，兼顾实时性与 CPU 占用。
